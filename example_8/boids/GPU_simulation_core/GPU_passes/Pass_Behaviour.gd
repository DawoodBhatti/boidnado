...
# ---------------------------------------------------------
# Public entry point
# N_boids = ceil(boid_count / local_size_x)
# ---------------------------------------------------------
func run(rd : RenderingDevice, compute_list : int, N_boids : int) -> void:
	if not _initialised:
		_init_pass(rd)

	if debug:
		print("\n[Behaviour] --- RUN ---")
		print("[Behaviour] workgroups (boids): ", N_boids)

	_print_dispatch_message()
	_run_behaviour(rd, compute_list, N_boids)
