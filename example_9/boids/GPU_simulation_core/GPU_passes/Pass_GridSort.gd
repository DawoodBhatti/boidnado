...
# Public entry point
# ---------------------------------------------------------
func run(rd : RenderingDevice, workgroups_boids : int, workgroups_cells : int) -> void:
	if not _initialised:
		_init_passes(rd)
	...
	# PASS 1: Histogram
	var list_hist : int = rd.compute_list_begin()
	_run_histogram(rd, list_hist, workgroups_boids)
	rd.compute_list_end()

	# PASS 2: Prefix
	var list_prefix : int = rd.compute_list_begin()
	_run_prefix(rd, list_prefix, workgroups_cells)
	rd.compute_list_end()

	# PASS 3: Scatter
	var list_scatter : int = rd.compute_list_begin()
	_run_scatter(rd, list_scatter, workgroups_boids)
	rd.compute_list_end()
