...
func _ready() -> void:
	...
	print("Example 9: moving from rd local -> rd global which has implicit sync?")

	#We discovered that there is some read/write desync
	#between compute shaders in the same pass
	
	# Using the global render thread should solve the desync issue.
