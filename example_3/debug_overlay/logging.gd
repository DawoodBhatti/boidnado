...
# ---------------------------------------------------------
# CONFIG (All directory paths centralized here)
# ---------------------------------------------------------

const DEBUG_PRINT: bool = false

# Base directory for logs (relative to project root)
const LOG_BASE_DIR := "res://%s/debug_overlay/"

# File name templates
const LOCAL_LOG_FILE := "local_performance_log_%d.json"
const LOCAL_SUMMARY_FILE := "local_performance_summary_%d.json"
...
func log_results(FPS: Label, CPU: Label, RAM: Label, Draws: Label, BoidCount: Label) -> void:
...
func get_summary() -> Dictionary:
	return {
		...
		"BoidCount": _summarize(samples["BoidCount"])
	}
...
func _write_local_raw(version: String, summary: Dictionary, runtime: float, timestamp: String) -> void:
	var iter := _get_iteration_number(version)
	var folder := LOG_BASE_DIR % version
	var path := folder + LOCAL_LOG_FILE % iter
...
func _write_local_summary(version: String, last_summary: Dictionary, runtime: float) -> void:
	...
	var log_path := folder + LOCAL_LOG_FILE % iter
	var summary_path := folder + LOCAL_SUMMARY_FILE % iter
	...
	var metrics := ["FPS", "CPU", "RAM", "BoidCount"]
...
func _debug_print_local(version: String) -> void:
	...
	var raw_path := folder + LOCAL_LOG_FILE % iter
	var summary_path := folder + LOCAL_SUMMARY_FILE % iter
